...
function addtocart() {
    ...
    Swal.fire({
        icon: 'success',
        title: 'เพิ่ม ' + product[productindex].name + ' ใส่รถเข็นแล้ว !'
    })
...
function rendercart() {
    if (cart.length > 0) {
        ...
        $("#mycart").html(`<p>ไม่มีรายการสินค้า</p>`)
    }
}
    ...
    if (action == '-') {
        if (cart[index].count > 0) {
            ...
            if (cart[index].count <= 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'คุณต้องการจะลบสินค้านี้หรือไม่',
        ...
        success: function(response) {
            ...
            if (response.RespCode == 200) {
                Swal.fire({
                    ...
                    html: ` <p> ราคาสินค้าทั้งหมด : ${response.Amount.Amount} บาท</p>
                            <p> ค่าจัดส่ง : ${response.Amount.Shipping} บาท</p>
                            <p> ภาษี : ${response.Amount.Vat} บาท</p>
                            <p> รวมทั้งสิ้น : ${response.Amount.Netamount} บาท</p>
                            `
                }).then((res) => {
                    if (res.isConfirmed) {
                        cart = [];
